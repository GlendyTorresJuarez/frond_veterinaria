...
const configCalendar = computed(() => {
    ...
    return {
        ...
        dateClick(info) {
            ...
            if (![2].includes(usuario.value.key_rol)) {
                eventoNuevo()
            }
        }
    }
...
const eventoDialogo = (id, event) => {
    ...
    if (event == 2) {
        iconConf.value = 'tabler-friends-off'
        menssageConf.value = 'Estas seguro de cambiar de estado a Ausencia del paciente? '
...
const eventoConfirmacion = async () => {
    ...
    var data = {
        key_estado: confEvent.value == 1 ? 7 : 8,
            ...
            <div class="sidebar">
                ...
                <div class="nav-02">
                    <div class="menu text-center">
                        <div v-if="[1, 3].includes(usuario.key_rol)">
                        ...
                        <div v-else class="py-4 px-4">
                            <h4 class="text-h6 text-uppercase font-weight-bold" style="font-weight: 300;">Todas las
                                citas
                            </h4>
                        </div>
                        <VDivider />
                ...
                <VForm @submit.prevent="eventoCrud()">
                    <VCardText>
                        <VRow>
                            ...
                            <VCol v-if="idCita == 0 && usuario.key_rol == 1" cols="12" sm="12" md="12">
        ...
        <VDialog v-model="modal[2]" max-width="600">
            ...
            <VCard title="Registrar mascota">
                <VForm @submit.prevent="nuevaMascota()">
                    <VCardText>
                        <VRow>
                            <VCol v-if="usuario.key_rol == 1" cols="12" sm="12" md="12">
        ...
        <div id="menu-calender" v-show="isMenuCalender">
            <VCard style="width:360px" v-for="items in itemsCitasId ">
                ...
                <VCardText class="py-1 px-5">
                    ...
                    <div class="w-100 mt-3 mb-2" v-if="items.dni_vt == usuario.documento || usuario.key_rol == 1">
                        <div class="mb-3">
                            ...
                            <VBtn v-if="[9, 8, 5].includes(items.key_estado)"
                                :style="[9].includes(items.key_estado) ? 'width:48.5%;' : 'width:100%;'" class="font-12"
                                variant="outlined" color="secundary" @click="eventoDialogo(items.id, 1)">
                                <VIcon icon="tabler-report-off"></VIcon> <span class="ms-2">Cancelar cita</span>
                            </VBtn>

                            <VBtn v-if="[9].includes(items.key_estado)" style="width:48.5%;" class="font-12 ms-2"
                                variant="outlined" color="secundary" @click="eventoDialogo(items.id, 2)">
                                <VIcon icon="tabler-friends-off"></VIcon> <span class="ms-2">Ausencia</span>
                            </VBtn>
                            <VBtn v-if="items.key_estado == 6" class="w-100 font-12" variant="outlined" color="info"
                                :to="{ name: 'citas-detalle', params: { detalle: encriptar(items.id) } }">
                        ...
                        <div v-if="[9, 8, 5].includes(items.key_estado)" class="w-100">
                            <VBtn class="font-12"
                                :style="[9, 5].includes(items.key_estado) ? 'width:48.5%;' : 'width:100%;'"
                                variant="outlined" color="secundary" @click="eventoReprogramarDuplicar(items, 1)">
                                <VIcon icon="tabler-calendar-stats"></VIcon> <span class="ms-2"> Reprogramar</span>
                            </VBtn>

                            <VBtn v-if="[9, 5].includes(items.key_estado)" style="width:48.5%;" class="ms-2 font-12"
                                variant="outlined" color="secundary" @click="eventoEditar(items)">
                    ...
                    <div class="w-100 mt-3 mb-2" v-if="items.dni_cl == usuario.documento && usuario.key_rol == 3">
                        <VBtn v-if="[9, 5].includes(items.key_estado)" class="font-12 mb-2 w-100" variant="outlined"
                            color="secundary" @click="eventoEditar(items)">
                            <VIcon icon="tabler-calendar-stats"></VIcon> <span class="ms-2"> editar</span>
                        </VBtn>
            ...
            <VCard>
                <VCardText>
                    <div class="d-flex justify-center align-center mt-3 mb-3 flex-column">
                        ...
                        <h6 class="text-lg font-weight-medium text-center">{{ menssageConf }}</h6>
                    </div>
                    <div v-if="subMenssage != '' && confEvent == 1">
                        <p>{{ subMenssage }}</p>
