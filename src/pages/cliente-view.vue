<script setup>
import { VDataTable, VToast } from "@/views/components";
...
const eventoNuevo = () => {
    ...
    titleCrud.value = "Registrar Clientes";
...
const eventoEditar = (items) => {
    ...
    titleCrud.value = "Actualizar datos del Clientes";
...
const eventoCrud = async () => {
    ...
    if (isVerificar && idCliente.value == 0) {
        loadings.value[2] = false
        return false
    }

    await axios({
        method: idCliente.value == 0 ? 'POST' : 'PUT',
        url: idCliente.value == 0 ? "backend-citas/agregar-clientes/" : `backend-citas/actualizar-clientes/${idCliente.value}/`,
        data: data
    }).then(res => {
        console.log(res);
        if (res.status == 200 && !res.data.is_error) {
            valueToas.value = {
                'status': true,
                'title': idCliente.value == 0 ? '¡Registro Creado 🥳!' : '¡Registro Actualizado 🥳!',
...
const eventoDialogo = (id, event) => {
    ...
    if (event == 1) {
        ...
        menssageConf.value = 'Estas seguro de anular al cliente?'
        colorConf.value = 'warning'
        confEvent.value = 1
    }
    if (event == 2) {
        iconConf.value = 'tabler-checks'
        menssageConf.value = 'Desea activar al cliente?'
...
const eventoConfirmacion = async () => {
    ...
    await axios({
        ...
        url: `backend-citas/recuperar-eliminar-clientes/${idCliente.value}/`,
        data: data
    }).then(res => {
        if (res.status == 200 && !res.data.is_error) {
            valueToas.value = {
...
const exportDoc = async () => {
    ...
    await axios({
        url: 'backend-citas/exportar-clientes/',
        method: 'POST',
...
<template>
    <div>
        <v-breadcrumbs class="mb-5" :items="['Inicio', 'Clientes']">
        ...
        <VRow>
            <VCol>
                <div class="table">
                    <VDataTable :type="'full'" :headers="header" :items="dataClientes" :loadings="loadings[0]"
                        :search="search">
