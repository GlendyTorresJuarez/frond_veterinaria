...
const eventoNuevo = () => {
    ...
    titleCrud.value = "Registrar tipo de mascota";
    btnTextoCrud.value = "Guardar";
    limpiar()
}

const limpiar = () => {
    idTipoMascota.value = 0
    tipo.value = ''
    descripcion.value = ''
}
...
const eventoEditar = (items) => {
    ...
    titleCrud.value = "Actualizar tipo de mascota";
    btnTextoCrud.value = "Actualizar";
    itemsCampos(items)
}

const itemsCampos = (items) => {
    idTipoMascota.value = items.id
    tipo.value = items.tipo
...
const eventoCrud = async () => {
    ...
    var isVerificar = tipo.value == ''

    if (isVerificar && idTipoMascota.value == 0) {
        loadings.value[1] = false
        return false
    }

    await axios({
        method: idTipoMascota.value == 0 ? 'POST' : 'PUT',
        url: idTipoMascota.value == 0 ? "backend-citas/agregar-tipo-mascota/" : `backend-citas/actualizar-tipo-mascota/${idTipoMascota.value}/`,
        data: data
    }).then(res => {
        if (res.status == 200 && !res.data.is_error) {
            valueToas.value = {
                'status': true,
                'title': idTipoMascota.value == 0 ? '¡Registro Creado 🥳!' : '¡Registro Actualizado 🥳!',
...
const eventoDialogo = (id, event) => {
    ...
    if (event == 1) {
        ...
        menssageConf.value = 'Estas seguro de anular el tipo de mascota?'
...
const eventoConfirmacion = async () => {
    ...
    await axios({
        ...
        url: `backend-citas/recuperar-eliminar-tipo-mascota/${idTipoMascota.value}/`,
        data: data
    }).then(res => {
        if (res.status == 200 && !res.data.is_error) {
            valueToas.value = {
...
const exportDoc = async () => {
    ...
    await axios({
        url: 'backend-citas/exportar-tipo-mascota/',
        method: 'POST',
        ...
        if (res.status == 200) {
            ...
            fileLink.setAttribute("download", "Tipo de mascota.xlsx");
...
<template>
    <div>
        <v-breadcrumbs class="mb-5" :items="['Inicio', 'Tipo de mascota']">
        ...
        <VRow>
            <VCol>
                <div class="table">
                    <VDataTable :type="'full'" :headers="header" :items="dataTipoMascota" :loadings="loadings[0]"
                        :search="search">
                    ...
                    <VCardText>
                        <VRow>
                            <VCol cols="12" sm="12" md="12">
                                <VTextField prepend-inner-icon="tabler-dog-bowl" v-model="tipo" label="Tipo"
