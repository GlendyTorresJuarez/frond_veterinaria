...
const eventoNuevo = () => {
    modal.value[0] = true;
    titleCrud.value = "Registrar tipo servicios";
...
const eventoEditar = (item) => {
    modal.value[0] = true;
    titleCrud.value = "Actualizar tipo servicios";
    btnTextoCrud.value = "Actualizar";
    itemsCampos(item)
}
const itemsCampos = (items) => {
    idTipoServicio.value = items.id
    tipoServicio.value = items.tipo_servicio
    descripcion.value = items.descripcion
}

const limpiar = () => {
    idTipoServicio.value = 0
    tipoServicio.value = ''
    descripcion.value = ''
}
...
const eventoCrud = async () => {
    ...
    var isVerificar = tipoServicio.value == ''

    if (isVerificar && idTipoServicio.value == 0) {
        loadings.value[1] = false
        return false
    }

    await axios({
        method: idTipoServicio.value == 0 ? 'POST' : 'PUT',
        url: idTipoServicio.value == 0 ? "backend-citas/agregar-tipo-servicios/" : `backend-citas/actualizar-tipo-servicios/${idTipoServicio.value}/`,
        data: data
    }).then(res => {
        if (res.status == 200 && !res.data.is_error) {
            valueToas.value = {
                'status': true,
                'title': idTipoServicio.value == 0 ? '¡Registro Creado 🥳!' : '¡Registro Actualizado 🥳!',
...
const eventoDialogo = (id, event) => {
    ...
    if (event == 1) {
        ...
        menssageConf.value = 'Estas seguro de anular el tipo de servicio?'
        colorConf.value = 'warning'
        confEvent.value = 1
    }
    if (event == 2) {
        iconConf.value = 'tabler-checks'
        menssageConf.value = 'Desea activar el tipo de servicio?'
        colorConf.value = 'success'
        confEvent.value = 2
    }
    if (event == 3) {
        iconConf.value = 'tabler-trash'
        menssageConf.value = 'Estas seguro de eliminar por completo el tipo servicio?'
        colorConf.value = 'error'
        confEvent.value = 3
    }
    idTipoServicio.value = id
}
...
const eventoConfirmacion = async () => {
    ...
    await axios({
        ...
        url: `backend-citas/recuperar-eliminar-tipo-servicios/${idTipoServicio.value}/`,
        data: data
    }).then(res => {
        if (res.status == 200 && !res.data.is_error) {
            valueToas.value = {
...
const exportDoc = async () => {
    ...
    await axios({
        url: 'backend-citas/exportar-tipo-servicios/',
        method: 'POST',
        ...
        if (res.status == 200) {
            ...
            fileLink.setAttribute("download", "Tipo de servicios.xlsx");
...
<template>
    <div>
        <v-breadcrumbs class="mb-5" :items="['Inicio', 'Tipo Servicios']">
        ...
        <VRow>
            <VCol>
                <div class="table">
                    <VDataTable :type="'full'" :headers="header" :items="dataTipoServicio" :loadings="loadings[0]"
                        :search="search">
                    ...
                    <VCardText>
                        <VRow>
                            <VCol cols="12" sm="12" md="12">
                                <VTextField prepend-inner-icon="tabler-tournament" v-model="tipoServicio"
                                    label="Tipo servicios" :rules="[(v) => !!v || 'Campo requerido']" />
                            </VCol>
                            <VCol cols="12" sm="12" md="12">
